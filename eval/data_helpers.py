...
#CSV 형태에 따라 INSTANCE 내 라벨이 달라집니다.
class Data_test:
    def __init__(self, file_instances):
        # Load data
        self.instances = self.read(file_instances)
    ...
    def read(self, filename):
        ...
        rows = []
        # Process file
        #with codecs.open(filename, "r") as table:
         #   r = DictReader(table, delimiter='\t')
        with codecs.open(filename, 'r', 'utf8') as table:
            r = DictReader(table)
            for line in r:
                rows.append(line)
        return rows
...
def tokenize(string):
    ...
    """
    final_string = ''
    tokenized = [''.join(t) for t in pos_tagger.pos(string) if t[1] in ['NNG','NNP','VV','VA','VCP','VCN','XSV','XSA']]
...
def load_data_and_labels_test(file_instances):
    """
    Loads MR polarity data from files, splits the data into words and generates labels.
    Returns split sentences and labels.
    """
    ...
    for instance in data.instances:
        news_id = instance['seqid']
        test_ids.append(news_id)
        test_heads.append(tokenize(data.headlines[news_id]))
        test_bodies.append(tokenize(data.bodies[news_id]))

    return test_ids, test_heads, test_bodies
