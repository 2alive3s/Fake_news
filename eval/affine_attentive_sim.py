...
# Combine all the pooled features
    def __init__(
      ...
      embedding_size, filter_sizes, num_filters, l2_reg_lambda=0.1):
        ...
        self.pooled_outputs_head_maxpool = []
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-head-%s" % filter_size):
                ...
                pooled_head = tf.nn.max_pool(
                    ...
                    padding='VALID',
                    name="pool")
                self.pooled_outputs_head_maxpool.append(pooled_head)

        self.pooled_outputs_body = []
        self.pooled_outputs_body_maxpool = []
        for i, filter_size in enumerate(filter_sizes):
            with tf.name_scope("conv-maxpool-body-%s" % filter_size):
                ...
                pooled_body = tf.nn.max_pool(
                    ...
                    padding='VALID',
                    name="pool")
                self.pooled_outputs_body_maxpool.append(pooled_body)
        ...
        self.pooled_outputs_head_maxpool_reshape = tf.reshape(self.pooled_outputs_head_maxpool, [-1, 256])
        self.pooled_outputs_body_maxpool_reshape = tf.reshape(self.pooled_outputs_body_maxpool, [-1, 1024])
        pooled_outputs = tf.concat([self.pooled_outputs_head_maxpool_reshape,self.sims,self.pooled_outputs_body_maxpool_reshape],-1,name='preconcat')
